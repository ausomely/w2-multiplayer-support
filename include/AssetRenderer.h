...
class CAssetRenderer
{
  ...
  public:
    ...
    void DrawOverlays(std::shared_ptr<CGraphicSurface> surface,
                      const SRectangle &rect);
    void DrawPlacement(std::shared_ptr<CGraphicSurface> surface,
                       const SRectangle &rect, const CPixelPosition &pos,
                       EAssetType type, std::shared_ptr<CPlayerAsset> builder);
