...
#endif
  public:
    CGraphicResourceContextCairo(cairo_t *rc, bool reference = false)
            : DResourceContext(rc), DWindow(nullptr)
    {
        if (reference)
        {
...
#if GTK_CHECK_VERSION(3, 22, 0)  //(GDK_VERSION_MAX_ALLOWED >= GDK_VERSION_3_22)
    CGraphicResourceContextCairo(GdkWindow *window,
                                 GdkDrawingContext *drawingcontext)
            : DWindow(window), DDrawingContext(drawingcontext)
    {
        DResourceContext =
            gdk_drawing_context_get_cairo_context(DDrawingContext);
    };
#endif
    ...
    void SetSourceSurface(std::shared_ptr<CGraphicSurface> srcsurface, int xpos,
                          int ypos) override;
    ...
    void Clip() override;
    void MaskSurface(std::shared_ptr<CGraphicSurface> srcsurface, int xpos,
                     int ypos) override;

    std::shared_ptr<CGraphicSurface> GetTarget() override;
    void Save() override;
    void Restore() override;
    void DrawSurface(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos,
                     int dypos, int width, int height, int sxpos,
                     int sypos) override;
    void CopySurface(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos,
                     int dypos, int width, int height, int sxpos,
                     int sypos) override;
};

class CGraphicSurfaceCairo
        : public CGraphicSurface,
          public std::enable_shared_from_this<CGraphicSurface>
{
  ...
  public:
    CGraphicSurfaceCairo(cairo_surface_t *surface, bool reference = false)
            : DSurface(surface)
    {
        if (reference)
        {
        ...
        if (DSurface)
        {
            switch (cairo_image_surface_get_format(DSurface))
            {
                case CAIRO_FORMAT_ARGB32:
                    return ESurfaceFormat::ARGB32;
                case CAIRO_FORMAT_RGB24:
                    return ESurfaceFormat::RGB24;
                case CAIRO_FORMAT_A8:
                    return ESurfaceFormat::A8;
                default:
    ...
    void Clear(int xpos = 0, int ypos = 0, int width = -1,
               int height = -1) override;
    std::shared_ptr<CGraphicSurface> Duplicate() override;

    std::shared_ptr<CGraphicResourceContext> CreateResourceContext() override;

    void Draw(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
              int width, int height, int sxpos, int sypos) override;
    void Copy(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
              int width, int height, int sxpos, int sypos) override;
    void CopyMaskSurface(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos,
                         int dypos,
                         std::shared_ptr<CGraphicSurface> masksurface,
                         int sxpos, int sypos) override;
    void Transform(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos,
                   int dypos, int width, int height, int sxpos, int sypos,
                   void *calldata,
                   TGraphicSurfaceTransformCallback callback) override;
};
