...
class CSoundEventRenderer
{
  protected:
    ...
    static float RightBias(const SRectangle &viewportrect,
                           const CPosition &position);
    static bool OnScreen(const SRectangle &viewportrect,
                         const CPosition &position);

  public:
    CSoundEventRenderer(std::shared_ptr<CSoundLibraryMixer> mixer,
