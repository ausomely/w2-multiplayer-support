...
class CSoundClip
{
  protected:
    ...
    std::shared_ptr<CDataSource> DLoadingSource;
    std::shared_ptr<CIOChannel> DLoadingChannel;
    mpg123_handle *DMPG123Handle;
    int DMPGEncoding;
    int DMPGChannels;
    ...
    static bool ChannelInputEvent(std::shared_ptr<CIOChannel> channel,
  ...
  public:
    ...
    bool Load(std::shared_ptr<CDataSource> source, bool ismp3 = false,
              bool asynchload = true);

    void CopyStereoClip(float *data, int offset, int frames);
    void MixStereoClip(float *data, int offset, int frames, float volume = 1.0,
                       float rightbias = 0.0, bool loop = false);
};
