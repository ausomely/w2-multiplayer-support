...
CPlayerData::CPlayerData(std::shared_ptr< CAssetDecoratedMap > map, EPlayerNumber number, EPlayerColor color){
    DIsAI = true;
    DGameCycle = 0;
    DNumber = number;
    DColor = color;
    ...
    for(auto &AssetInit : DActualMap->AssetInitializationList()){
        if(AssetInit.DNumber == number){
            PrintDebug(DEBUG_LOW, "Init %s %d (%d, %d)\n", AssetInit.DType.c_str(), AssetInit.DNumber, AssetInit.DTilePosition.X(), AssetInit.DTilePosition.Y());
...
std::weak_ptr< CPlayerAsset > CPlayerData::FindNearestEnemy(const CPixelPosition &pos, int range){
    ...
    for(auto Asset : DPlayerMap->Assets()){
        if((Asset->Number() != DNumber)&&(Asset->Number() != EPlayerNumber::Neutral)&&(Asset->Alive())){
...
CGameModel::CGameModel(int mapindex, uint64_t seed, const std::array< EPlayerColor, to_underlying(EPlayerNumber::Max)> &newcolors){
    ...
    for(int PlayerIndex = 0; PlayerIndex < to_underlying(EPlayerNumber::Max); PlayerIndex++){
        DPlayers.at(PlayerIndex) = std::make_shared< CPlayerData > (DActualMap, static_cast< EPlayerNumber >(PlayerIndex), newcolors[PlayerIndex]);
    }
    ...
    for(auto &Asset : AllAssets){
        ...
        if(EAssetAction::Capability == Asset->Action()){
            ...
            if(Command.DActivatedCapability){
                ...
                Asset->PopCommand();
                if(PlayerCapability->CanApply(Asset, DPlayers.at(to_underlying(Asset->Number())), Command.DAssetTarget)){
                    PlayerCapability->ApplyCapability(Asset, DPlayers.at(to_underlying(Asset->Number())), Command.DAssetTarget);
                }
                else{
            ...
            if(EDirection::Max == HarvestDirection){
                if(TilePosition == Asset->TilePosition()){
                    CTilePosition TilePosition = DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->FindNearestReachableTileType(Asset->TilePosition(), CTerrainMap::ETileType::Forest);
                    // Find new lumber
                    Asset->PopCommand();
                    if(0 <= TilePosition.X()){
                        CPixelPosition NewPosition;
                        NewPosition.SetFromTile(TilePosition);
                        Command.DAssetTarget = DPlayers.at(to_underlying(Asset->Number()))->CreateMarker(NewPosition, false);
                ...
                if(DHarvestSteps <= Asset->Step()){
                    std::weak_ptr< CPlayerAsset > NearestRepository = DPlayers.at(to_underlying(Asset->Number()))->FindNearestOwnedAsset(Asset->Position(), {EAssetType::TownHall, EAssetType::Keep, EAssetType::Castle, EAssetType::LumberMill});
            ...
            if((EDirection::Max == MineDirection)&&(TilePosition != Asset->TilePosition())){
                ...
                if(0 == Asset->Step()){
                    ...
                    if(DMineSteps <= Asset->Step()){
                        ...
                        std::weak_ptr< CPlayerAsset > NearestRepository = DPlayers.at(to_underlying(Asset->Number()))->FindNearestOwnedAsset(Asset->Position(), {EAssetType::TownHall, EAssetType::Keep, EAssetType::Castle});
                        CTilePosition NextTarget(DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->Width()-1,DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->Height()-1);
                        ...
                        Asset->TilePosition(DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->FindAssetPlacement(Asset, OldTarget, NextTarget));
                    }
                }
            }
            ...
            SAssetCommand Command = Asset->CurrentCommand();
            auto NewTarget = DPlayers.at(to_underlying(Asset->Number()))->FindNearestEnemy(Asset->Position(), Asset->EffectiveRange());
            ...
            if(CurrentCommand.DAssetTarget->Alive()){
                ...
                if(EDirection::Max == RepairDirection){
                    ...
                    if(Asset->Step() == Asset->AttackSteps()){
                        if(DPlayers.at(to_underlying(Asset->Number()))->Gold() && DPlayers.at(to_underlying(Asset->Number()))->Lumber()){
                            ...
                            DPlayers.at(to_underlying(Asset->Number()))->DecrementGold(1);
                            DPlayers.at(to_underlying(Asset->Number()))->DecrementLumber(1);
                            ...
                            if(CurrentCommand.DAssetTarget->HitPoints() == CurrentCommand.DAssetTarget->MaxHitPoints()){
                                ...
                                TempEvent.DAsset = Asset;
                                DPlayers.at(to_underlying(Asset->Number()))->AddGameEvent(TempEvent);
                                Asset->PopCommand();
                            }
                        }
                        else{
            ...
            if(EAssetType::None == Asset->Type()){
                ...
                if(CPosition::HalfTileWidth() * CPosition::HalfTileHeight() > Asset->Position().DistanceSquared(ClosestTargetPosition)){
                    ...
                    if(CurrentCommand.DAssetTarget->Alive()){
                        ...
                        DPlayers.at(to_underlying(CurrentCommand.DAssetTarget->Number()))->AddGameEvent(TempEvent);
                            ...
                            if(!CurrentCommand.DAssetTarget->Alive()){
                                ...
                                if((EAssetAction::Capability == Command.DAction)&&(Command.DAssetTarget)){
                                    if(EAssetAction::Construct == Command.DAssetTarget->Action()){
                                        DPlayers.at(to_underlying(Command.DAssetTarget->Number()))->DeleteAsset(Command.DAssetTarget);
                                    }
                                }
                    ...
                    if(ClosestTargetPosition.DistanceSquared( Asset->Position() ) > RangeToDistanceSquared(Asset->EffectiveRange())){
                        ...
                        */
                        ...
                        if(Asset->Step() == Asset->AttackSteps()){
                            ...
                            auto ArrowAsset = DPlayers.at(to_underlying(EPlayerNumber::Neutral))->CreateAsset("None");
                            int Damage = Asset->EffectiveBasicDamage() - CurrentCommand.DAssetTarget->EffectiveArmor();
                            Damage = 0 > Damage ? 0 : Damage;
                            Damage += Asset->EffectivePiercingDamage();
                            if(DRandomNumberGenerator.Random() & 0x1){ // 50% chance half damage
                ...
                Asset->PopCommand();
                if(EAssetAction::StandGround != NextCommand.DAction){
                    auto NewTarget = DPlayers.at(to_underlying(Asset->Number()))->FindNearestEnemy(Asset->Position(), Asset->EffectiveSight());
            ...
            if(DConveySteps <= Asset->Step()){
                ...
                DPlayers.at(to_underlying(Asset->Number()))->IncrementGold(Asset->Gold());
                DPlayers.at(to_underlying(Asset->Number()))->IncrementLumber(Asset->Lumber());
                Asset->Gold(0);
                Asset->Lumber(0);
                Asset->PopCommand();
                Asset->ResetStep();
                ...
                Asset->TilePosition(DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->FindAssetPlacement(Asset, Command.DAssetTarget, NextTarget));
            }
            ...
            if(Asset->Step() > DDeathSteps){
                if(Asset->Speed()){
                    ...
                    // Create corpse
                    auto CorpseAsset = DPlayers.at(to_underlying(EPlayerNumber::Neutral))->CreateAsset("None");
        ...
        if(EAssetAction::Walk == Asset->Action()){
            if(Asset->TileAligned()){
                ...
                TravelDirection = DRouterMap.FindRoute(*DPlayers.at(to_underlying(Asset->Number()))->PlayerMap(), *Asset, MapTarget);
                if(EDirection::Max != TravelDirection){
                    ...
                    if((TilePosition == Asset->TilePosition())||(EDirection::Max != Asset->TilePosition().AdjacentTileDirection(TilePosition))){
                        ...
                        TilePosition = DPlayers.at(to_underlying(Asset->Number()))->PlayerMap()->FindNearestReachableTileType(Asset->TilePosition(), CTerrainMap::ETileType::Forest);
                        // Find new lumber
                        Asset->PopCommand();
                        Asset->PopCommand();
                        if(0 <= TilePosition.X()){
