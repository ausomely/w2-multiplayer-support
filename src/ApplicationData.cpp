...
void CApplicationData::Activate(){
    ...
    if(!DSoundLibraryMixer->LoadLibrary(TempDataSource, SoundLoadingCallback, this)){
...
void CApplicationData::LoadGameMap(int index){
    ...
    for(int Index = 1; Index < to_underlying(EPlayerNumber::Max); Index++){
        DGameModel->Player(static_cast<EPlayerNumber>(Index))->IsAI((ptAIEasy <= DLoadingPlayerTypes[Index])&&(ptAIHard >= DLoadingPlayerTypes[Index]));        
    }
    for(int Index = 1; Index < to_underlying(EPlayerNumber::Max); Index++){
        if(DGameModel->Player(static_cast<EPlayerNumber>(Index))->IsAI()){
            ...
            DAIPlayers[Index] = std::make_shared< CAIPlayer > (DGameModel->Player(static_cast<EPlayerNumber>(Index)), Downsample);
        }
    ...
    DMapRenderer = std::make_shared< CMapRenderer >(std::make_shared< CMemoryDataSource >(DMapRendererConfigurationData), DTerrainTileset, DGameModel->Player(DPlayerNumber)->PlayerMap()); 
    PrintDebug(DEBUG_LOW, "Creating AssetRenderer\n");
    DAssetRenderer = std::make_shared< CAssetRenderer >(DAssetRecolorMap, DAssetTilesets, DMarkerTileset, DCorpseTileset, DFireTilesets, DBuildingDeathTileset, DArrowTileset, DGameModel->Player(DPlayerNumber), DGameModel->Player(DPlayerNumber)->PlayerMap() );
    PrintDebug(DEBUG_LOW, "Creating FogRenderer\n");
    DFogRenderer = std::make_shared< CFogRenderer >(DFogTileset, DGameModel->Player(DPlayerNumber)->VisibilityMap());
    ...
    DUnitDescriptionRenderer = std::make_shared< CUnitDescriptionRenderer >(DMiniBevel, DIconTileset, DFonts, DGameModel->Player(DPlayerNumber)->Color());
    PrintDebug(DEBUG_LOW, "Creating CUnitActionRenderer\n");
    DUnitActionRenderer = std::make_shared< CUnitActionRenderer >(DMiniBevel, DIconTileset, DGameModel->Player(DPlayerNumber));
    PrintDebug(DEBUG_LOW, "Creating CResourceRenderer\n");
    DResourceRenderer = std::make_shared< CResourceRenderer > (DMiniIconTileset, DFonts[to_underlying(CUnitDescriptionRenderer::EFontSize::Medium)], DGameModel->Player(DPlayerNumber));
    PrintDebug(DEBUG_LOW, "Creating CSoundEventRenderer\n");
    DSoundEventRenderer = std::make_shared< CSoundEventRenderer > (DSoundLibraryMixer, DGameModel->Player(DPlayerNumber));
    ...
    DMenuButtonRenderer->ButtonColor(DGameModel->Player(DPlayerNumber)->Color());
