...
int CSoundLibraryMixer::ClipDurationMS(int index)
{
    ...
    return (DSoundClips[index].TotalFrames() * 1000) /
           DSoundClips[index].SampleRate();
}

int CSoundLibraryMixer::TimestepCallback(
...
int CSoundLibraryMixer::Timestep(void *out, unsigned long frames,
                                 ...
                                 PaStreamCallbackFlags status)
{
    ...
    DMutex.lock();
    for (std::list<SClipStatus>::iterator ClipIterator =
             DClipsInProgress.begin();
         ClipIterator != DClipsInProgress.end();)
    {
        bool Advance = true;

        DSoundClips[ClipIterator->DIndex].MixStereoClip(
            DataPtr, ClipIterator->DOffset, frames, ClipIterator->DVolume,
            ClipIterator->DRightBias);

        ClipIterator->DOffset += frames;
        if (ClipIterator->DOffset >=
            DSoundClips[ClipIterator->DIndex].TotalFrames())
        {
    ...
    {
        DMusicClips[DMusicStatus.DIndex].MixStereoClip(
            DataPtr, DMusicStatus.DOffset, frames, DMusicStatus.DVolume,
            DMusicStatus.DRightBias, true);

        DMusicStatus.DOffset = (DMusicStatus.DOffset + frames) %
                               DMusicClips[DMusicStatus.DIndex].TotalFrames();
    }

    for (std::list<SToneStatus>::iterator ToneIterator =
             DTonesInProgress.begin();
         ToneIterator != DTonesInProgress.end();)
    {
        bool Advance = true;
        ...
        for (int Frame = 0; Frame < frames; Frame++)
        {
            ...
            *CurOutPtr++ += ToneIterator->DVolume *
                            (1.0 - ToneIterator->DRightBias) *
                            DSineWave[SineIndex];
            *CurOutPtr++ += ToneIterator->DVolume *
                            (1.0 + ToneIterator->DRightBias) *
                            DSineWave[SineIndex];
...
bool CSoundLibraryMixer::LoadLibrary(std::shared_ptr<CDataSource> source,
                                     TSoundLibraryLoadingCallback callback,
                                     TSoundLibraryLoadingCalldata calldata)
{
    ...
    {
        DSineWave[Index] =
            sin(((double) Index / (double) DSampleRate) * M_PI * 2.0);
    }
    ...
    if (!DPortAudioInitialized)
    {
        ...
        PrintDebug(DEBUG_LOW,
                   "Port Audio not initialized, try to reinitialize.\n");
        ...
        {
            PrintDebug(DEBUG_LOW,
                       "Port Audio failed to initialize with code %d.\n",
                       PortAudioResult);
        }
    }
    PrintDebug(DEBUG_LOW, "Opening stream at %dHz with %d frames per buffer\n",
               DSampleRate, FRAMES_PER_BUFFER);
    if (paNoError != Pa_OpenDefaultStream(&DStream, 0, 2, paFloat32,
                                          DSampleRate, FRAMES_PER_BUFFER,
                                          TimestepCallback, this))
    {
    ...
    for (int Index = 0; Index < TotalClips; Index++)
    {
        ...
        if (!DSoundClips[Index].Load(
                DSoundDataContainer->DataSource(TempString)))
        {
...
int CSoundLibraryMixer::PlayTone(float freq, float freqdecay, float volume,
                                 float volumedecay, float rightbias,
                                 float rightshift)
{
