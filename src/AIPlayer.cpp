...
bool CAIPlayer::SearchMap(SPlayerCommandRequest &command){
    ...
    if(MovableAsset){
        CTilePosition UnknownPosition = DPlayerData->PlayerMap()->FindNearestReachableTileType(MovableAsset->TilePosition(), CTerrainMap::ETileType::None);
...
bool CAIPlayer::BuildTownHall(SPlayerCommandRequest &command){
    ...
    if(BuilderAsset){
        auto GoldMineAsset = DPlayerData->FindNearestAsset(BuilderAsset->Position(), EAssetType::GoldMine);
        CTilePosition Placement = DPlayerData->FindBestAssetPlacement(GoldMineAsset->TilePosition(), BuilderAsset, EAssetType::TownHall, 1);
        if(0 <= Placement.X()){
...
bool CAIPlayer::BuildBuilding(SPlayerCommandRequest &command, EAssetType buildingtype, EAssetType neartype){
    ...
    if(BuilderAsset){
        ...
        CTilePosition MapCenter(DPlayerData->PlayerMap()->Width()/2, DPlayerData->PlayerMap()->Height()/2);
        ...
        CTilePosition Placement = DPlayerData->FindBestAssetPlacement(SourcePosition, BuilderAsset, buildingtype, 1);
        if(0 > Placement.X()){
            return SearchMap(command);
        }
        if(PlayerCapability){
...
bool CAIPlayer::ActivatePeasants(SPlayerCommandRequest &command, bool trainmore){
    ...
    for(auto WeakAsset : DPlayerData->Assets()){
        if(auto Asset = WeakAsset.lock()){
            ...
            if(Asset->HasCapability(EAssetCapabilityType::BuildPeasant) && (EAssetAction::None == Asset->Action())){
                TownHallAsset = Asset;
            }
        }
    }
    ...
    if(MiningAsset || (InterruptibleAsset && (SwitchToLumber || SwitchToGold))){
        if(MiningAsset && (MiningAsset->Lumber() || MiningAsset->Gold())){
            ...
            auto GoldMineAsset = DPlayerData->FindNearestAsset(MiningAsset->Position(), EAssetType::GoldMine);
            if(GoldMiners && ((DPlayerData->Gold() > DPlayerData->Lumber() * 3) || SwitchToLumber)){
                CTilePosition LumberLocation = DPlayerData->PlayerMap()->FindNearestReachableTileType(MiningAsset->TilePosition(), CTerrainMap::ETileType::Forest);
        ...
        auto PlayerCapability = CPlayerCapability::FindCapability(EAssetCapabilityType::BuildPeasant); 
...
bool CAIPlayer::TrainFootman(SPlayerCommandRequest &command){
    ...
    if(TrainingAsset){
        auto PlayerCapability = CPlayerCapability::FindCapability(EAssetCapabilityType::BuildFootman); 
...
void CAIPlayer::CalculateCommand(SPlayerCommandRequest &command){
    ...
    if((DCycle % DDownSample) == 0){
        ...
        if(0 == DPlayerData->FoundAssetCount(EAssetType::GoldMine)){
            ...
            int BarracksCount = 0;
            int FootmanCount = DPlayerData->PlayerAssetCount(EAssetType::Footman);
            int ArcherCount = DPlayerData->PlayerAssetCount(EAssetType::Archer)+DPlayerData->PlayerAssetCount(EAssetType::Ranger);
            
            if(!CompletedAction && (DPlayerData->FoodConsumption() >= DPlayerData->FoodProduction())){
                CompletedAction = BuildBuilding(command, EAssetType::Farm, EAssetType::Farm);
            }
            if(!CompletedAction){
            ...
            if(!CompletedAction && (0 == (BarracksCount = DPlayerData->PlayerAssetCount(EAssetType::Barracks)))){
                CompletedAction = BuildBuilding(command, EAssetType::Barracks, EAssetType::Farm);
            }
            ...
            if(!CompletedAction && (0 == DPlayerData->PlayerAssetCount(EAssetType::LumberMill))){
                CompletedAction = BuildBuilding(command, EAssetType::LumberMill, EAssetType::Barracks);
            }
