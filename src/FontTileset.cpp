...
bool CFontTileset::LoadFont(std::shared_ptr<CGraphicRecolorMap> colormap,
                            std::shared_ptr<CDataSource> source)
{
    ...
    for (int Index = 0; Index < DTileCount; Index++)
    {
        ...
        DBottomOpaque = 0;
        DSearchCall = 0;
        DSurfaceTileset->Transform(DSurfaceTileset, 0, Index * DTileHeight,
                                   DTileWidth, DTileHeight, 0,
                                   Index * DTileHeight, this, TopBottomSearch);
...
void CFontTileset::DrawText(std::shared_ptr<CGraphicSurface> surface, int xpos,
                            int ypos, const std::string &str)
{
    ...
    for (int Index = 0; Index < str.length(); Index++)
    {
        ...
        {
            xpos +=
                DCharacterWidths[LastChar] + DDeltaWidths[LastChar][NextChar];
        }
...
void CFontTileset::DrawTextColor(std::shared_ptr<CGraphicSurface> surface,
                                 int xpos, int ypos, int colorindex,
                                 const std::string &str)
{
    ...
    for (int Index = 0; Index < str.length(); Index++)
    {
        ...
        CGraphicMulticolorTileset::DrawTile(surface, xpos, ypos, NextChar,
                                            colorindex);
...
void CFontTileset::DrawTextWithShadow(std::shared_ptr<CGraphicSurface> surface,
                                      int xpos, int ypos, int color,
                                      int shadowcol, int shadowwidth,
                                      const std::string &str)
{
    if ((0 > color) || (color >= DColoredTilesets.size()))
    {
        PrintDebug(DEBUG_HIGH, "  Invalid color %d of %zd\n", color,
                   DColoredTilesets.size());
        return;
    }
    ...
    DrawTextColor(surface, xpos + shadowwidth, ypos + shadowwidth, shadowcol,
                  str);
...
void CFontTileset::MeasureTextDetailed(const std::string &str, int &width,
