...
{
    cairo_set_source_rgba(DResourceContext,
                          ((double) ((rgba >> 16) & 0xFF)) / 255.0,
                          ((double) ((rgba >> 8) & 0xFF)) / 255.0,
                          ((double) (rgba & 0xFF)) / 255.0,
                          ((double) ((rgba >> 24) & 0xFF)) / 255.0);
}

void CGraphicResourceContextCairo::SetSourceRGBA(double r, double g, double b,
                                                 double a)
{
...
void CGraphicResourceContextCairo::SetSourceSurface(
    std::shared_ptr<CGraphicSurface> srcsurface, int xpos, int ypos)
{
    ...
    CairoSourceSurface =
        std::dynamic_pointer_cast<CGraphicSurfaceCairo>(srcsurface);
    cairo_set_source_surface(DResourceContext, CairoSourceSurface->DSurface,
                             xpos, ypos);
}
...
void CGraphicResourceContextCairo::Rectangle(int xpos, int ypos, int width,
                                             int height)
{
...
void CGraphicResourceContextCairo::MaskSurface(
    std::shared_ptr<CGraphicSurface> srcsurface, int xpos, int ypos)
{
    std::shared_ptr<CGraphicSurfaceCairo> CairoSourceSurface;
    ...
    CairoSourceSurface =
        std::dynamic_pointer_cast<CGraphicSurfaceCairo>(srcsurface);
    cairo_mask_surface(DResourceContext, CairoSourceSurface->DSurface, xpos,
                       ypos);
}
...
std::shared_ptr<CGraphicSurface> CGraphicResourceContextCairo::GetTarget()
{
    return std::make_shared<CGraphicSurfaceCairo>(
        cairo_get_target(DResourceContext), true);
}
...
void CGraphicResourceContextCairo::DrawSurface(
    std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
    int width, int height, int sxpos, int sypos)
{
    std::shared_ptr<CGraphicSurfaceCairo> CairoSourceSurface;
    ...
    cairo_set_source_surface(DResourceContext, CairoSourceSurface->DSurface,
                             dxpos - sxpos, dypos - sypos);
    cairo_rectangle(DResourceContext, dxpos, dypos, width, height);
    cairo_fill(DResourceContext);
}

void CGraphicResourceContextCairo::CopySurface(
    std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
    int width, int height, int sxpos, int sypos)
{
    std::shared_ptr<CGraphicSurfaceCairo> CairoSourceSurface;
...
std::shared_ptr<CGraphicResourceContext> CGraphicSurfaceCairo::
    CreateResourceContext()
{
    if (DSurface)
    {
        return std::shared_ptr<CGraphicResourceContext>(
            new CGraphicResourceContextCairo(cairo_create(DSurface)));
    }
    return nullptr;
}
...
uint32_t CGraphicSurfaceCairo::PixelAt(int xpos, int ypos)
{
    ...
    {
        PrintDebug(DEBUG_HIGH,
                   "Ivalid Pixel (%d, %d) out of range of (%d x %d)\n", xpos,
                   ypos, Width(), Height());
        return 0;
    }
    ...
    switch (cairo_image_surface_get_format(DSurface))
    {
        case CAIRO_FORMAT_ARGB32:
            Pixel = (uint32_t *) (SrcPixels + (Stride * ypos));
            Pixel += xpos;
            return *Pixel;
...
void CGraphicSurfaceCairo::Clear(int xpos, int ypos, int width, int height)
{
    ...
    std::shared_ptr<CGraphicResourceContextCairo> CairoResourceContext =
        std::dynamic_pointer_cast<CGraphicResourceContextCairo>(
            CreateResourceContext());

    cairo_save(CairoResourceContext->DResourceContext);
    cairo_rectangle(CairoResourceContext->DResourceContext, xpos, ypos, width,
                    height);
    cairo_set_operator(CairoResourceContext->DResourceContext,
...
std::shared_ptr<CGraphicSurface> CGraphicSurfaceCairo::Duplicate()
{
    if (DSurface)
    {
        ...
        std::shared_ptr<CGraphicSurface> NewSurface(new CGraphicSurfaceCairo(
...
void CGraphicSurfaceCairo::Draw(std::shared_ptr<CGraphicSurface> srcsurface,
                                int dxpos, int dypos, int width, int height,
                                int sxpos, int sypos)
{
    ...
    cairo_set_source_surface(CairoResourceContext->DResourceContext,
                             CairoSourceSurface->DSurface, dxpos - sxpos,
                             dypos - sypos);
    cairo_rectangle(CairoResourceContext->DResourceContext, dxpos, dypos, width,
                    height);
    cairo_fill(CairoResourceContext->DResourceContext);
}

void CGraphicSurfaceCairo::Copy(std::shared_ptr<CGraphicSurface> srcsurface,
                                int dxpos, int dypos, int width, int height,
                                int sxpos, int sypos)
{
    std::shared_ptr<CGraphicResourceContext> ResourceContext;
...
void CGraphicSurfaceCairo::CopyMaskSurface(
    std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
    std::shared_ptr<CGraphicSurface> masksurface, int sxpos, int sypos)
{
    std::shared_ptr<CGraphicResourceContext> ResourceContext;
    std::shared_ptr<CGraphicResourceContextCairo> CairoResourceContext;
    std::shared_ptr<CGraphicSurfaceCairo> CairoSourceSurface;
    ...
    CairoMaskSurface =
        std::dynamic_pointer_cast<CGraphicSurfaceCairo>(masksurface);
    if (!CairoMaskSurface)
    {
        return;
    }
    ...
    cairo_set_source_surface(CairoResourceContext->DResourceContext,
                             CairoSourceSurface->DSurface, dxpos - sxpos,
                             dypos - sypos);
    cairo_mask_surface(CairoResourceContext->DResourceContext,
                       CairoMaskSurface->DSurface, dxpos, dypos);
    cairo_set_operator(CairoResourceContext->DResourceContext,
                       CAIRO_OPERATOR_SOURCE);
    cairo_fill(CairoResourceContext->DResourceContext);
    cairo_restore(CairoResourceContext->DResourceContext);
...
void CGraphicSurfaceCairo::Transform(
    std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos,
    int width, int height, int sxpos, int sypos, void *calldata,
    TGraphicSurfaceTransformCallback callback)
{
    std::shared_ptr<CGraphicSurfaceCairo> CairoSourceSurface;
    ...
    switch (cairo_image_surface_get_format(CairoSourceSurface->DSurface))
    {
        case CAIRO_FORMAT_ARGB32:
            ...
            {
                uint32_t *Pixel =
                    (uint32_t *) (SrcPixels + (Stride * (sypos + Row)));
                Pixel += sxpos;
        ...
        case CAIRO_FORMAT_RGB24:
            for (int Row = 0; Row < height; Row++)
            {
                ...
                for (int Col = 0; Col < width; Col++)
                {
                    BufferedPixels[Row].push_back(
                        callback(calldata, (*Pixel) | 0xFF000000));
                    Pixel++;
                }
            }
            break;
    ...
    switch (cairo_image_surface_get_format(CairoSourceSurface->DSurface))
    {
        ...
        case CAIRO_FORMAT_RGB24:
            ...
            {
                uint32_t *Pixel =
                    (uint32_t *) (DestPixels + (Stride * (dypos + Row)));
                Pixel += dxpos;
...
std::shared_ptr<CGraphicSurface> CGraphicFactory::CreateSurface(
    int width, int height, CGraphicSurface::ESurfaceFormat format)
{
    cairo_format_t Format;
    switch (format)
    {
        case CGraphicSurface::ESurfaceFormat::ARGB32:
            Format = CAIRO_FORMAT_ARGB32;
            break;
        case CGraphicSurface::ESurfaceFormat::RGB24:
            Format = CAIRO_FORMAT_RGB24;
            break;
        case CGraphicSurface::ESurfaceFormat::A8:
            Format = CAIRO_FORMAT_A8;
            break;
        case CGraphicSurface::ESurfaceFormat::A1:
    ...
    return std::shared_ptr<CGraphicSurface>(new CGraphicSurfaceCairo(
        cairo_image_surface_create(Format, width, height)));
}

cairo_status_t GraphicFactoryCairoDataRead(void *closure, unsigned char *data,
                                           unsigned int length)
{
    CDataSource *DataSource = static_cast<CDataSource *>(closure);

    return DataSource->Read(data, length) ? CAIRO_STATUS_SUCCESS
                                          : CAIRO_STATUS_READ_ERROR;
}

std::shared_ptr<CGraphicSurface> CGraphicFactory::LoadSurface(
    std::shared_ptr<CDataSource> source)
{
    return std::shared_ptr<CGraphicSurface>(
        new CGraphicSurfaceCairo(cairo_image_surface_create_from_png_stream(
            GraphicFactoryCairoDataRead, source.get())));
}

cairo_status_t GraphicFactoryCairoDataWrite(void *closure,
                                            const unsigned char *data,
                                            unsigned int length)
{
    CDataSink *DataSink = static_cast<CDataSink *>(closure);

    return length == DataSink->Write(data, length) ? CAIRO_STATUS_SUCCESS
                                                   : CAIRO_STATUS_READ_ERROR;
}

bool CGraphicFactory::StoreSurface(std::shared_ptr<CDataSink> sink,
                                   std::shared_ptr<CGraphicSurface> surface)
{
    ...
    CairoSourceSurface =
        std::dynamic_pointer_cast<CGraphicSurfaceCairo>(surface);
    return cairo_surface_write_to_png_stream(
               CairoSourceSurface->DSurface, GraphicFactoryCairoDataWrite,
               sink.get()) == CAIRO_STATUS_SUCCESS;
}
