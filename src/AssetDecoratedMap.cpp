...
std::weak_ptr< CPlayerAsset > CAssetDecoratedMap::FindNearestAsset(const CPixelPosition &pos, EPlayerNumber number, EAssetType type){
    std::shared_ptr< CPlayerAsset > BestAsset;
    int BestDistanceSquared = -1;
    
    for(auto &Asset : DAssets){
        if((Asset->Type() == type)&&(Asset->Number() == number)&&(EAssetAction::Construct != Asset->Action())){
...
bool CAssetDecoratedMap::LoadMap(std::shared_ptr< CDataSource > source){
    ...
    try{
        ...
        for(int Index = 0; Index <= ResourceCount; Index++){
            ...
            TempResourceInit.DNumber = static_cast<EPlayerNumber>(std::stoi(Tokens[0]));
            if((0 == Index)&&(EPlayerNumber::Neutral != TempResourceInit.DNumber)){
                PrintError("Expected first resource to be for player neutral.\n");
                goto LoadMapExit;
            }
        ...
        for(int Index = 0; Index < AssetCount; Index++){
            ...
            TempAssetInit.DNumber = static_cast<EPlayerNumber>(std::stoi(Tokens[1]));
