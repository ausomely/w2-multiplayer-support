...
bool CGraphicTileset::ParseGroupName(const std::string &tilename,
                                     std::string &aniname, int &anistep)
{
...
int CGraphicTileset::TileCount(int count)
{
    ...
    auto TempSurface = CGraphicFactory::CreateSurface(
        DTileWidth, count * DTileHeight, DSurfaceTileset->Format());
...
bool CGraphicTileset::DuplicateTile(int destindex, const std::string &tilename,
                                    int srcindex)
{
    if ((0 > srcindex) || (0 > destindex) || (srcindex >= DTileCount) ||
        (destindex >= DTileCount))
    {
        return false;
    }
    if (tilename.empty())
    ...
    DSurfaceTileset->Copy(DSurfaceTileset, 0, destindex * DTileHeight,
                          DTileWidth, DTileHeight, 0, srcindex * DTileHeight);
...
bool CGraphicTileset::DuplicateClippedTile(int destindex,
                                           const std::string &tilename,
                                           int srcindex, int clipindex)
{
    if ((0 > srcindex) || (0 > destindex) || (0 > clipindex) ||
        (srcindex >= DTileCount) || (destindex >= DTileCount) ||
        (clipindex >= DClippingMasks.size()))
    {
        return false;
    }
    if (tilename.empty())
    ...
    ClearTile(destindex);
    DSurfaceTileset->CopyMaskSurface(
        DSurfaceTileset, 0, destindex * DTileHeight, DClippingMasks[clipindex],
        0, srcindex * DTileHeight);
    auto OldMapping = DMapping.find(DTileNames[destindex]);
    if (DMapping.end() != OldMapping)
    {
        DMapping.erase(OldMapping);
    ...
    DClippingMasks[destindex] = CGraphicFactory::CreateSurface(
        DTileWidth, DTileHeight, CGraphicSurface::ESurfaceFormat::A1);
    DClippingMasks[destindex]->Copy(DSurfaceTileset, 0, 0, DTileWidth,
                                    DTileHeight, 0, destindex * DTileHeight);
...
void CGraphicTileset::CreateClippingMasks()
{
    if (DSurfaceTileset)
    {
        ...
        for (int Index = 0; Index < DTileCount; Index++)
        {
            DClippingMasks[Index] = CGraphicFactory::CreateSurface(
                DTileWidth, DTileHeight, CGraphicSurface::ESurfaceFormat::A1);
            DClippingMasks[Index]->Copy(DSurfaceTileset, 0, 0, DTileWidth,
                                        DTileHeight, 0, Index * DTileHeight);
        }
    }
}
...
bool CGraphicTileset::LoadTileset(std::shared_ptr<CDataSource> source)
{
    ...
    DSurfaceTileset =
        CGraphicFactory::LoadSurface(source->Container()->DataSource(PNGPath));
...
void CGraphicTileset::DrawTile(std::shared_ptr<CGraphicSurface> surface,
                               int xpos, int ypos, int tileindex)
{
    ...
    surface->Draw(DSurfaceTileset, xpos, ypos, DTileWidth, DTileHeight, 0,
                  tileindex * DTileHeight);
}

void CGraphicTileset::DrawClipped(std::shared_ptr<CGraphicSurface> surface,
                                  int xpos, int ypos, int tileindex,
                                  uint32_t rgb)
{
