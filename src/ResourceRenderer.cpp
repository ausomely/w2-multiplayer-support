...
CResourceRenderer::CResourceRenderer(std::shared_ptr<CGraphicTileset> icons,
                                     ...
                                     std::shared_ptr<CPlayerData> player)
{
    ...
    DIconIndices[to_underlying(EMiniIconTypes::Gold)] =
        DIconTileset->FindTile("gold");
    DIconIndices[to_underlying(EMiniIconTypes::Lumber)] =
...
void CResourceRenderer::DrawResources(std::shared_ptr<CGraphicSurface> surface)
{
    ...
    DFont->DrawTextWithShadow(
        ...
        std::string(" ") +
            CTextFormatter::IntegerToPrettyString(DLastGoldDisplay));
    ...
    if (DPlayer->FoodConsumption() > DPlayer->FoodProduction())
    {
        ...
        // DFont->MeasureText( std::string(" ") +
        // std::to_string(DPlayer->FoodConsumption()), FirstTextWidth,
        // TextHeight);
        DFont->MeasureText(
            std::string(" / ") + std::to_string(DPlayer->FoodProduction()),
            SecondTextWidth, TextHeight);
        DFont->MeasureText(
            std::string(" ") + std::to_string(DPlayer->FoodConsumption()) +
                std::string(" / ") + std::to_string(DPlayer->FoodProduction()),
            TotalTextWidth, TextHeight);
        DFont->DrawTextWithShadow(
            surface, XOffset + DIconTileset->TileWidth(), TextYOffset,
            DInsufficientColor, DBackgroundColor, 1,
            std::string(" ") + std::to_string(DPlayer->FoodConsumption()));
        DFont->DrawTextWithShadow(
            surface,
            XOffset + DIconTileset->TileWidth() + TotalTextWidth -
                SecondTextWidth,
            TextYOffset, DForegroundColor, DBackgroundColor, 1,
            std::string(" / ") + std::to_string(DPlayer->FoodProduction()));
    }
    else
    {
