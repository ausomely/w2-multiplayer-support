...
bool CPath::DecomposePath(std::vector<std::string> &decomp,
                          const std::string &path)
{
    ...
    decomp.clear();
    Anchor = 0;
    while ((Anchor + 1 < path.length()) &&
           (-1 != (DelimiterLocation = path.find(DDelimiterString, Anchor))))
    {
...
CPath CPath::SimplifyPath(const CPath &srcpath, const CPath &destpath)
{
    ...
    while (Index < RelPath.size())
    {
        ...
        else if (RelPath[Index] == "..")
        {
            ...
            {
                ReturnPath.DDecomposedPath.erase(
                    ReturnPath.DDecomposedPath.end() - 1);
                RelPath.erase(RelPath.begin());
            }
            else
            {
    ...
    ReturnPath.DDecomposedPath.insert(ReturnPath.DDecomposedPath.end(),
...
CPath CPath::RelativePath(const CPath &srcpath, const CPath &destpath)
{
    ...
    else
    {
        ...
        while (ReturnPath.DDecomposedPath.size() && BasePath.size())
        {
            ...
            {
                ReturnPath.DDecomposedPath.erase(
                    ReturnPath.DDecomposedPath.begin());
        ...
        while (BasePath.size())
        {
            ReturnPath.DDecomposedPath.insert(
                ReturnPath.DDecomposedPath.begin(), "..");
            BasePath.erase(BasePath.begin());
        }
    }
