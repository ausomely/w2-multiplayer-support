            ...
            if(EAssetCapabilityType::BuildSimple == context->DCurrentAssetCapability){
                ...
                if(KeyLookup != context->DBuildHotKeyMap.end()){
                    ...
                    if(PlayerCapability){
                        ...
                        if(PlayerCapability->CanInitiate(ActorTarget, context->DGameModel->Player(context->DPlayerNumber))){
                            SGameEvent TempEvent;
                            TempEvent.DType = EEventType::ButtonTick;
                            context->DGameModel->Player(context->DPlayerNumber)->AddGameEvent(TempEvent);
                    ...
                    if(HasCapability){
                        ...
                        if(PlayerCapability){
                            if((CPlayerCapability::ETargetType::None == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::Player == PlayerCapability->TargetType())){
                                ...
                                if(PlayerCapability->CanApply(ActorTarget, context->DGameModel->Player(context->DPlayerNumber), ActorTarget)){
                                    
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = KeyLookup->second;
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DActors = context->DSelectedPlayerAssets;
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetNumber = EPlayerNumber::Neutral;
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType = EAssetType::None;
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation = ActorTarget->Position();
                                    context->DCurrentAssetCapability = EAssetCapabilityType::None;
                                }
                            }
                            else{ 
        ...
        if(context->DRightClick && !context->DRightDown && context->DSelectedPlayerAssets.size()){
            ...
            if(CanMove){
                if(EPlayerNumber::Neutral != PixelType.Number()){
                    ...
                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Move;
                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetNumber = PixelType.Number();
                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType = PixelType.AssetType();
                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DActors = context->DSelectedPlayerAssets;
                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation = TempPosition;
                    if(PixelType.Number() == context->DPlayerNumber){
                        ...
                        if(HaveGold){
                            if((EAssetType::TownHall == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)||(EAssetType::Keep == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)||(EAssetType::Castle == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)){
                                context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Convey;
                            }
                        }
                        else if(HaveLumber){
                            if((EAssetType::TownHall == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)||(EAssetType::Keep == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)||(EAssetType::Castle == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)||(EAssetType::LumberMill == context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType)){
                                context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Convey;
                            }   
                        }
                        else{
                            auto TargetAsset = context->DGameModel->Player(context->DPlayerNumber)->SelectAsset(TempPosition, PixelType.AssetType()).lock();
                            if((0 == TargetAsset->Speed())&&(TargetAsset->MaxHitPoints() > TargetAsset->HitPoints())){
                                context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Repair;
                            }
                        }
                    }                                                                                                       
                    else{
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Attack;     
                    }
                    context->DCurrentAssetCapability = EAssetCapabilityType::None;
                }
                else{                
                    ...
                    if(CanHarvest){
                        if(CPixelType::EAssetTerrainType::Tree == PixelType.Type()){
                            ...
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Mine;
                            TempTilePosition.SetFromPixel(context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation);
                            if(CTerrainMap::ETileType::Forest != context->DGameModel->Player(context->DPlayerNumber)->PlayerMap()->TileType(TempTilePosition)){
                                // Could be tree pixel, but tops of next row
                                TempTilePosition.IncrementY(1);
                                if(CTerrainMap::ETileType::Forest == context->DGameModel->Player(context->DPlayerNumber)->PlayerMap()->TileType(TempTilePosition)){
                                    context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation.SetFromTile(TempTilePosition);
                                }
                            }
                        }
                        else if(CPixelType::EAssetTerrainType::GoldMine == PixelType.Type()){
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = EAssetCapabilityType::Mine;
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType = EAssetType::GoldMine;
                        }
                    }
                    context->DCurrentAssetCapability = EAssetCapabilityType::None;
                }
            ...
            if((EAssetCapabilityType::None == context->DCurrentAssetCapability)||(EAssetCapabilityType::BuildSimple == context->DCurrentAssetCapability)){
                    ...
                    if(ShiftPressed){
                        if(!context->DSelectedPlayerAssets.empty()){
                            if(auto TempAsset = context->DSelectedPlayerAssets.front().lock()){
                                if(TempAsset->Number() != context->DPlayerNumber){
                                    context->DSelectedPlayerAssets.clear();
                                }
                            }
                        }
                        context->DSelectedPlayerAssets.splice(context->DSelectedPlayerAssets.end(), context->DGameModel->Player(SearchNumber)->SelectAssets(TempRectangle, PixelType.AssetType(), 2 == context->DLeftClick));
                        ...
                        context->DSelectedPlayerAssets = context->DGameModel->Player(SearchNumber)->SelectAssets(TempRectangle, PixelType.AssetType(), 2 == context->DLeftClick);
                    }
                    for(auto WeakAsset : context->DSelectedPlayerAssets){
                        if(auto LockedAsset = WeakAsset.lock()){
                ...
                if(PlayerCapability && !context->DLeftDown){
                    if(((CPlayerCapability::ETargetType::Asset == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::TerrainOrAsset == PlayerCapability->TargetType()))&&(EAssetType::None != PixelType.AssetType())){
                        auto NewTarget = context->DGameModel->Player(PixelType.Number())->SelectAsset(TempPosition, PixelType.AssetType()).lock();
                        
                        if(PlayerCapability->CanApply(context->DSelectedPlayerAssets.front().lock(), context->DGameModel->Player(context->DPlayerNumber), NewTarget)){
                            SGameEvent TempEvent;
                            TempEvent.DType = EEventType::PlaceAction;
                            TempEvent.DAsset = NewTarget;
                            context->DGameModel->Player(context->DPlayerNumber)->AddGameEvent(TempEvent);
                            
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = context->DCurrentAssetCapability;
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DActors = context->DSelectedPlayerAssets;
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetNumber = PixelType.Number();
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType = PixelType.AssetType();
                            context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation = TempPosition;
                    ...
                    else if(((CPlayerCapability::ETargetType::Terrain == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::TerrainOrAsset == PlayerCapability->TargetType()))&&((EAssetType::None == PixelType.AssetType())&&(EPlayerNumber::Neutral == PixelType.Number()))){
                        auto NewTarget = context->DGameModel->Player(context->DPlayerNumber)->CreateMarker(TempPosition, false);
            ...
            if(PlayerCapability){
                if((CPlayerCapability::ETargetType::None == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::Player == PlayerCapability->TargetType())){
                    ...
                    if(PlayerCapability->CanApply(ActorTarget, context->DGameModel->Player(context->DPlayerNumber), ActorTarget)){
                        ...
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DAction = CapabilityType;
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DActors = context->DSelectedPlayerAssets;
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetNumber = EPlayerNumber::Neutral;
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetType = EAssetType::None;
                        context->DPlayerCommands[to_underlying(context->DPlayerNumber)].DTargetLocation = ActorTarget->Position();
...
void CBattleMode::Calculate(std::shared_ptr< CApplicationData > context){
    ...
    for(int Index = 1; Index < to_underlying(EPlayerColor::Max); Index++){
        if(context->DGameModel->Player(static_cast<EPlayerNumber>(Index))->IsAlive() && context->DGameModel->Player(static_cast<EPlayerNumber>(Index))->IsAI()){
            ...
            if(PlayerCapability){
                ...
                if((CPlayerCapability::ETargetType::None != PlayerCapability->TargetType())&&(CPlayerCapability::ETargetType::Player != PlayerCapability->TargetType())){
                    if(EAssetType::None == context->DPlayerCommands[Index].DTargetType){
                        NewTarget = context->DGameModel->Player(static_cast<EPlayerNumber>(Index))->CreateMarker(context->DPlayerCommands[Index].DTargetLocation, true);
                    }
                    else{
                        NewTarget = context->DGameModel->Player(context->DPlayerCommands[Index].DTargetNumber)->SelectAsset(context->DPlayerCommands[Index].DTargetLocation, context->DPlayerCommands[Index].DTargetType).lock();
                    }
                }
                
                for(auto &WeakActor : context->DPlayerCommands[Index].DActors){
                    if(auto Actor = WeakActor.lock()){
                        if(PlayerCapability->CanApply(Actor, context->DGameModel->Player(static_cast<EPlayerNumber>(Index)), NewTarget) && (Actor->Interruptible() || (EAssetCapabilityType::Cancel == context->DPlayerCommands[Index].DAction))){
                            PlayerCapability->ApplyCapability(Actor, context->DGameModel->Player(static_cast<EPlayerNumber>(Index)), NewTarget);
                        }
                    }
                }
            }
...
void CBattleMode::Render(std::shared_ptr< CApplicationData > context){
    ...
    for(auto Asset : context->DGameModel->Player(context->DPlayerNumber)->PlayerMap()->Assets()){
                                                            ...
                                                            if(PlayerCapability){
                                                                ...
                                                                if(EAssetType::None == PixelType.AssetType()){
                                                                    if((CPlayerCapability::ETargetType::Terrain == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::TerrainOrAsset == PlayerCapability->TargetType())){
                                                                        auto NewTarget = context->DGameModel->Player(context->DPlayerNumber)->CreateMarker(context->ViewportToDetailedMap(ViewportCursorLocation), false);
                                                    
                                                                        CanApply = PlayerCapability->CanApply(context->DSelectedPlayerAssets.front().lock(), context->DGameModel->Player(context->DPlayerNumber), NewTarget);  
                                                                    }
                                                                }
                                                                else{
                                                                    if((CPlayerCapability::ETargetType::Asset == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::TerrainOrAsset == PlayerCapability->TargetType())){
                                                                        auto NewTarget = context->DGameModel->Player(PixelType.Number())->SelectAsset(context->ViewportToDetailedMap(ViewportCursorLocation), PixelType.AssetType()).lock();
