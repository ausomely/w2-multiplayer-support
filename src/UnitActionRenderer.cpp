...
CUnitActionRenderer::CUnitActionRenderer(std::shared_ptr<CBevel> bevel,
                                         std::shared_ptr<CGraphicTileset> icons,
                                         std::shared_ptr<CPlayerData> player)
{
    DIconTileset = icons;
    DBevel = bevel;
    DPlayerData = player;
    DPlayerColor = player->Color();
    ...
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildPeasant)] =
    ...
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildBarracks)] =
        DIconTileset->FindTile("human-barracks");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildLumberMill)] =
        DIconTileset->FindTile("human-lumber-mill");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildBlacksmith)] =
        DIconTileset->FindTile("human-blacksmith");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildKeep)] =
        DIconTileset->FindTile("keep");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildCastle)] =
        DIconTileset->FindTile("castle");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildScoutTower)] =
        DIconTileset->FindTile("scout-tower");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildGuardTower)] =
        DIconTileset->FindTile("human-guard-tower");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildCannonTower)] =
        DIconTileset->FindTile("human-cannon-tower");
    DCommandIndices[to_underlying(EAssetCapabilityType::Move)] =
        DIconTileset->FindTile("human-move");
    DCommandIndices[to_underlying(EAssetCapabilityType::Repair)] =
        DIconTileset->FindTile("repair");
    DCommandIndices[to_underlying(EAssetCapabilityType::Mine)] =
        DIconTileset->FindTile("mine");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildSimple)] =
        DIconTileset->FindTile("build-simple");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildAdvanced)] =
        DIconTileset->FindTile("build-advanced");
    DCommandIndices[to_underlying(EAssetCapabilityType::Convey)] =
        DIconTileset->FindTile("human-convey");
    DCommandIndices[to_underlying(EAssetCapabilityType::Cancel)] =
        DIconTileset->FindTile("cancel");
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildWall)] =
        DIconTileset->FindTile("human-wall");
    DCommandIndices[to_underlying(EAssetCapabilityType::Attack)] =
        DIconTileset->FindTile("human-weapon-1");
    DCommandIndices[to_underlying(EAssetCapabilityType::StandGround)] =
        DIconTileset->FindTile("human-armor-1");
    DCommandIndices[to_underlying(EAssetCapabilityType::Patrol)] =
        DIconTileset->FindTile("human-patrol");
    DCommandIndices[to_underlying(EAssetCapabilityType::WeaponUpgrade1)] =
        DIconTileset->FindTile("human-weapon-1");
    DCommandIndices[to_underlying(EAssetCapabilityType::WeaponUpgrade2)] =
        DIconTileset->FindTile("human-weapon-2");
    DCommandIndices[to_underlying(EAssetCapabilityType::WeaponUpgrade3)] =
        DIconTileset->FindTile("human-weapon-3");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArrowUpgrade1)] =
        DIconTileset->FindTile("human-arrow-1");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArrowUpgrade2)] =
        DIconTileset->FindTile("human-arrow-2");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArrowUpgrade3)] =
        DIconTileset->FindTile("human-arrow-3");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArmorUpgrade1)] =
        DIconTileset->FindTile("human-armor-1");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArmorUpgrade2)] =
        DIconTileset->FindTile("human-armor-2");
    DCommandIndices[to_underlying(EAssetCapabilityType::ArmorUpgrade3)] =
...
{
    if (((pos.X() % (DFullIconWidth + DBevel->Width())) < DFullIconWidth) &&
        ((pos.Y() % (DFullIconHeight + DBevel->Width())) < DFullIconHeight))
    {
        int Index = (pos.X() / (DFullIconWidth + DBevel->Width())) +
                    (pos.Y() / (DFullIconHeight + DBevel->Width())) * 3;
        return DDisplayedCommands[Index];
    }
    return EAssetCapabilityType::None;
}

void CUnitActionRenderer::DrawUnitAction(
    std::shared_ptr<CGraphicSurface> surface,
    const std::list<std::weak_ptr<CPlayerAsset> > &selectionlist,
    EAssetCapabilityType currentaction)
{
    bool AllSame = true;
    bool IsFirst = true;
    bool Moveable = true;
    ...
    if (EAssetCapabilityType::None == currentaction)
    {
        if (Moveable)
        {
            DDisplayedCommands[0] = HasCargo ? EAssetCapabilityType::Convey
                                             : EAssetCapabilityType::Move;
            ...
            if (auto Asset = selectionlist.front().lock())
            {
                ...
                if (Asset->HasCapability(EAssetCapabilityType::BuildSimple) &&
                    (1 == selectionlist.size()))
                {
    ...
    for (auto IconType : DDisplayedCommands)
    {
        if (EAssetCapabilityType::None != IconType)
        {
            ...
            DBevel->DrawBevel(surface, XOffset, YOffset,
                              DIconTileset->TileWidth(),
                              DIconTileset->TileHeight());
            DIconTileset->DrawTile(surface, XOffset, YOffset,
                                   DCommandIndices[to_underlying(IconType)]);
            if (PlayerCapability)
            {
                if (!PlayerCapability->CanInitiate(selectionlist.front().lock(),
                                                   DPlayerData))
                {
                    DIconTileset->DrawTile(surface, XOffset, YOffset,
                                           DDisabledIndex);
