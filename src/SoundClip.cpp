...
bool CSoundClip::ChannelInputEvent(std::shared_ptr<CIOChannel> channel, SIOEventType &event, TIOCalldata data){
    auto ThisCoundClip = static_cast< CSoundClip * >(data);
    unsigned char Buffer[10240];
    int DataBytesRead = ThisCoundClip->DLoadingSource->Read(Buffer, sizeof(Buffer));
    int ReturnValue;
    
    
    if(0 < DataBytesRead){ 
        ReturnValue = mpg123_feed(ThisCoundClip->DMPG123Handle, Buffer, DataBytesRead);
        if(MPG123_OK == ReturnValue){
            size_t BytesRead;
            if(sizeof(float) == mpg123_encsize(ThisCoundClip->DMPGEncoding)){
                float FloatBuffer[1024];
                while(((ReturnValue = mpg123_read(ThisCoundClip->DMPG123Handle, (unsigned char *)FloatBuffer, sizeof(FloatBuffer), &BytesRead)) == MPG123_OK) && (BytesRead > 0)){
                    ...
                    if(1 == ThisCoundClip->DMPGChannels){
                        ...
                        ThisCoundClip->DData.insert(ThisCoundClip->DData.end(), FloatBuffer, FloatBuffer + Samples);
                    }
                } 
            }
            else if(sizeof(double) == mpg123_encsize(ThisCoundClip->DMPGEncoding)){
                double DoubleBuffer[1024];
                while(((ReturnValue = mpg123_read(ThisCoundClip->DMPG123Handle, (unsigned char *)DoubleBuffer, sizeof(DoubleBuffer), &BytesRead)) == MPG123_OK) && (BytesRead > 0)){
...
bool CSoundClip::Load(std::shared_ptr< CDataSource > source, bool ismp3, bool asynchload){
    if(ismp3){
        ...
        ReturnValue = mpg123_param(DMPG123Handle, MPG123_REMOVE_FLAGS, MPG123_IGNORE_INFOFRAME, 0.0);
        if(MPG123_OK == ReturnValue){
            ReturnValue = mpg123_param(DMPG123Handle, MPG123_ADD_FLAGS, MPG123_FORCE_FLOAT, 0.0);
            if(MPG123_OK == ReturnValue){
                ...
                if(MPG123_OK == ReturnValue){
                    ...
                    do{
                        DataBytesRead = source->Read(DataBuffer.data(), DataBuffer.size());
                        if(0 < DataBytesRead){
                            ReturnValue = mpg123_feed(DMPG123Handle, DataBuffer.data(), DataBytesRead);
                            if(MPG123_OK != ReturnValue){
                                break;
                            }
                        }
                    }while(MPG123_OK != mpg123_getformat(DMPG123Handle, &Rate, &DMPGChannels, &DMPGEncoding));
                    if(MPG123_OK == ReturnValue){
                        DChannels = 2;
                        DSampleRate = Rate;
                        ...
                        if(asynchload){
                            if(sizeof(float) == mpg123_encsize(DMPGEncoding)){
                                ...
                                DData.clear();
                                BytesRead = 0;
                                while(((ReturnValue = mpg123_read(DMPG123Handle, (unsigned char *)FloatBuffer, sizeof(FloatBuffer), &BytesRead)) != MPG123_OK)||(BytesRead == 0)){
                                    DataBytesRead = source->Read(DataBuffer.data(), DataBuffer.size());
                                    if(0 < DataBytesRead){
                                        ReturnValue = mpg123_feed(DMPG123Handle, DataBuffer.data(), DataBytesRead);
                                        if(MPG123_OK != ReturnValue){
                                ...
                                if(MPG123_OK == ReturnValue){
                                    ...
                                    if(1 == DMPGChannels){
                                        ...
                                        DData.insert(DData.end(), FloatBuffer, FloatBuffer + Samples);
                                    }
                                    ReturnStatus = true;
                                } 
                            }
                            else if(sizeof(double) == mpg123_encsize(DMPGEncoding)){
                                double DoubleBuffer[1024];
                                DData.clear();
                                BytesRead = 0;
                                while(((ReturnValue = mpg123_read(DMPG123Handle, (unsigned char *)DoubleBuffer, sizeof(DoubleBuffer), &BytesRead)) != MPG123_OK)||(BytesRead == 0)){
                                    DataBytesRead = source->Read(DataBuffer.data(), DataBuffer.size());
                                    if(0 < DataBytesRead){
                                        ReturnValue = mpg123_feed(DMPG123Handle, DataBuffer.data(), DataBytesRead);
                                        if(MPG123_OK != ReturnValue){
                                ...
                                if(MPG123_OK == ReturnValue){
                                    ...
                                    if(1 == DMPGChannels){
                                        for(int Index = 0; Index < Samples; Index++){
                                            DData.push_back(DoubleBuffer[Index]);
                                            DData.push_back(DoubleBuffer[Index]);
                                        }
                                    }
                                    else{
                            ...
                            if(ReturnStatus){
                                ...
                                DLoadingChannel->SetInEventCallback(this, ChannelInputEvent);
                            ...
                            if(MPG123_OK == ReturnValue){
                                if(sizeof(float) == mpg123_encsize(DMPGEncoding)){
                                    float Buffer[1024];
                                    DData.clear();
                                    while(((ReturnValue = mpg123_read(DMPG123Handle, (unsigned char *)Buffer, sizeof(Buffer), &BytesRead)) == MPG123_OK) && (BytesRead > 0)){
                                        int Samples = BytesRead / sizeof(float);
                                        if(1 == DMPGChannels){
                                            for(int Index = 0; Index < Samples; Index++){
