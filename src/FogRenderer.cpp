...
CFogRenderer::CFogRenderer(std::shared_ptr<CGraphicTileset> tileset,
                           std::shared_ptr<CVisibilityMap> map)
{
    DTileset = tileset;
    DMap = map;
    ...
    for (int Index = 0; Index < 0x100; Index++)
    {
        ...
        TempStringStream << std::setfill('0') << std::hex << std::uppercase
                         << std::setw(2) << Index;

        DFogIndices.push_back(
            DTileset->FindTile(std::string("pf-") + TempStringStream.str()));
        DBlackIndices.push_back(
            DTileset->FindTile(std::string("pb-") + TempStringStream.str()));
    }
    DSeenIndex = DFogIndices[0x00];
    DNoneIndex = DBlackIndices[0x00];
}

void CFogRenderer::DrawMap(std::shared_ptr<CGraphicSurface> surface,
                           const SRectangle &rect)
{
         ...
         YPos < rect.DHeight; YIndex++, YPos += TileHeight)
    {
        ...
        {
            CVisibilityMap::ETileVisibility TileType =
            ...
            if ((CVisibilityMap::ETileVisibility::Seen == TileType) ||
                (CVisibilityMap::ETileVisibility::SeenPartial == TileType))
            {
                DTileset->DrawTile(surface, XPos, YPos, DSeenIndex);
            }
            if ((CVisibilityMap::ETileVisibility::PartialPartial == TileType) ||
                (CVisibilityMap::ETileVisibility::Partial == TileType))
            {
                ...
                for (int YOff = -1; YOff < 2; YOff++)
                {
                    for (int XOff = -1; XOff < 2; XOff++)
                    {
                        if (YOff || XOff)
                        {
                            CVisibilityMap::ETileVisibility VisTile =
                                DMap->TileType(XIndex + XOff, YIndex + YOff);

                            if (CVisibilityMap::ETileVisibility::Visible ==
                                VisTile)
                            {
                ...
                DTileset->DrawTile(surface, XPos, YPos,
                                   DFogIndices[VisibilityIndex]);
            }
                ...
                for (int YOff = -1; YOff < 2; YOff++)
                {
                    for (int XOff = -1; XOff < 2; XOff++)
                    {
                        if (YOff || XOff)
                        {
                            ...
                            if ((CVisibilityMap::ETileVisibility::Visible ==
                                 VisTile) ||
                                (CVisibilityMap::ETileVisibility::Partial ==
                                 VisTile) ||
                                (CVisibilityMap::ETileVisibility::Seen ==
                                 VisTile))
                            {
                                VisibilityIndex |= VisibilityMask;
                            }
    ...
    for (int YPos = 0; YPos < DMap->Height(); YPos++)
    {
        ...
        while (XPos < DMap->Width())
        {
            ...
            if (CVisibilityMap::ETileVisibility::Visible != TileType)
            {
                ...
                switch (TileType)
                {
                    case CVisibilityMap::ETileVisibility::None:
                        ColorRGBA = 0xFF000000;
                        break;
                    case CVisibilityMap::ETileVisibility::Seen:
                    case CVisibilityMap::ETileVisibility::SeenPartial:
