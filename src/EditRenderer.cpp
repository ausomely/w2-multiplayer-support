...
CEditRenderer::CEditRenderer(std::shared_ptr<CGraphicRecolorMap> colors,
...
void CEditRenderer::DrawEdit(std::shared_ptr<CGraphicSurface> surface, int x,
                             int y, int cursorpos)
{
    ...
    ResourceContext->SetSourceRGBA(DColorMap->ColorValue(
        0 <= cursorpos ? DLightIndices[to_underlying(DBackgroundColor)]
                       : DDarkIndices[to_underlying(DBackgroundColor)],
        0));
    ResourceContext->Rectangle(x, y, DWidth, DHeight);
    ResourceContext->Fill();

    // DColorMap->DrawTileRectangle(drawable, x, y, DWidth, DHeight, 0 <=
    // cursorpos ? DLightIndices[DBackgroundColor] :
    // DDarkIndices[DBackgroundColor]);
    if (0 <= cursorpos)
    {
        if (DText.length())
        {
    ...
    DFont->DrawTextWithShadow(surface, x + BevelWidth, y + BevelWidth,
                              TextColorIndex, DBlackIndex, 1, RenderText);
    DInnerBevel->DrawBevel(surface, x + BevelWidth, y + BevelWidth,
                           DWidth - BevelWidth * 2, DHeight - BevelWidth * 2);
}
