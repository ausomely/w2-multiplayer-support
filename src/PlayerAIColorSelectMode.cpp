    ...
    if(context->DLeftClick && !context->DLeftDown){
        for(int Index = 0; Index < DColorButtonLocations.size(); Index++){
            if(DColorButtonLocations[Index].PointInside(CurrentX, CurrentY)){
                int PlayerSelecting = 1 + (Index / (to_underlying(EPlayerNumber::Max) - 1));
                int ColorSelecting = 1 + (Index % (to_underlying(EPlayerColor::Max) - 1));
                if((PlayerSelecting == to_underlying(context->DPlayerNumber))||(CApplicationData::gstMultiPlayerClient != context->DGameSessionType)){
                    if((PlayerSelecting == to_underlying(context->DPlayerNumber))||(CApplicationData::ptHuman != context->DLoadingPlayerTypes[PlayerSelecting])){
                        DPlayerNumberRequestingChange = static_cast<EPlayerNumber>(PlayerSelecting);
        ...
        for(int Index = 0; Index < DPlayerTypeButtonLocations.size(); Index++){
            if(DPlayerTypeButtonLocations[Index].PointInside(CurrentX, CurrentY)){
                DPlayerNumberRequesTypeChange = static_cast<EPlayerNumber>(Index+2);
                break;
            }
...
void CPlayerAIColorSelectMode::Calculate(std::shared_ptr< CApplicationData > context){   
    if(EPlayerNumber::Neutral != DPlayerNumberRequestingChange){
        ...
        if(EPlayerNumber::Neutral != NewColorInUseBy){
            context->DLoadingPlayerColors[to_underlying(NewColorInUseBy)] = context->DLoadingPlayerColors[to_underlying(DPlayerNumberRequestingChange)];
        }
        context->DLoadingPlayerColors[to_underlying(DPlayerNumberRequestingChange)] = DPlayerColorChangeRequest;
                         
        context->DAssetRenderer->UpdateLoadingPlayerColors(context->DLoadingPlayerColors);
    }
    if(EPlayerNumber::Neutral != DPlayerNumberRequesTypeChange){
        if(CApplicationData::gstSinglePlayer == context->DGameSessionType){
            switch(context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)]){
                case CApplicationData::ptAIEasy:    context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIMedium;
                                                    break;
                case CApplicationData::ptAIMedium:  context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIHard;
                                                    break;
                default:                            context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIEasy;
                                                    break;
            }
        }
        else if(CApplicationData::gstMultiPlayerHost == context->DGameSessionType){
            switch(context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)]){
                case CApplicationData::ptHuman:     context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIEasy;
                                                    break;
                case CApplicationData::ptAIEasy:    context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIMedium;
                                                    break;
                case CApplicationData::ptAIMedium:  context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptAIHard;
                                                    break;
                case CApplicationData::ptAIHard:    context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptNone;
                                                    break;
                default:                            context->DLoadingPlayerTypes[to_underlying(DPlayerNumberRequesTypeChange)] = CApplicationData::ptHuman;
                                                    break;
            }
        }
    }
...
void CPlayerAIColorSelectMode::Render(std::shared_ptr< CApplicationData > context){   
    ...
    for(int Index = 1; Index <= context->DSelectedMap->PlayerCount(); Index++){
        ...
        context->DFonts[to_underlying(CUnitDescriptionRenderer::EFontSize::Large)]->MeasureText(TempString, TextWidth, TextHeight);
        context->DFonts[to_underlying(CUnitDescriptionRenderer::EFontSize::Large)]->DrawTextWithShadow(context->DWorkingBufferSurface, context->DBorderWidth, TextTop, Index == to_underlying(context->DPlayerNumber) ? GoldColor : WhiteColor, ShadowColor, 1, TempString);
