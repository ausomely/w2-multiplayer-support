...
std::shared_ptr<CIOChannel> CIOFactory::NewFileIOChannel(
    const std::string &filename, const struct SFileOptions &options,
    std::shared_ptr<CDataSource> src, std::shared_ptr<CDataSink> sink)
{
    return std::make_shared<CIOChannelFileGlib>(
        open(filename.c_str(), options.DValue, 0600), src, sink);
}

std::shared_ptr<CIOChannel> CIOFactory::NewFileDescriptorIOChannel(
    int fd, std::shared_ptr<CDataSource> src, std::shared_ptr<CDataSink> sink)
{
    return std::make_shared<CIOChannelFileGlib>(fd, src, sink);
}

CIOChannelFileGlib::CIOChannelFileGlib(int fd, std::shared_ptr<CDataSource> src,
                                       std::shared_ptr<CDataSink> sink)
{
    ...
    Flags &= CIOFactory::SFileOptions::ReadOnly |
             CIOFactory::SFileOptions::WriteOnly |
             CIOFactory::SFileOptions::ReadWrite;
    DDataSource = src;
    if (!DDataSource && ((CIOFactory::SFileOptions::ReadOnly == Flags) ||
                         (CIOFactory::SFileOptions::ReadWrite == Flags)))
    {
        DDataSource = std::make_shared<CFileDataSource>("", DFileHandle);
    }
    DDataSink = sink;
    if (!DDataSink && ((CIOFactory::SFileOptions::WriteOnly == Flags) ||
                       (CIOFactory::SFileOptions::ReadWrite == Flags)))
    {
...
gboolean CIOChannelFileGlib::InEventCallback(GIOChannel *source,
                                             ...
                                             gpointer data)
{
    ...
    if (Channel->DInCallback)
    {
        ...
        EventType.SetIn();
        if (Channel->DInCallback(Channel->shared_from_this(), EventType,
                                 Channel->DInCalldata))
        {
            return TRUE;
        }
...
gboolean CIOChannelFileGlib::OutEventCallback(GIOChannel *source,
                                              GIOCondition condition,
                                              gpointer data)
{
    CIOChannelFileGlib *Channel = static_cast<CIOChannelFileGlib *>(data);
    ...
    if (Channel->DOutCallback)
    {
        ...
        EventType.SetOut();
        if (Channel->DOutCallback(Channel->shared_from_this(), EventType,
                                  Channel->DOutCalldata))
        {
            return TRUE;
        }
    }
...
gboolean CIOChannelFileGlib::PriorityEventCallback(GIOChannel *source,
                                                   GIOCondition condition,
                                                   gpointer data)
{
    CIOChannelFileGlib *Channel = static_cast<CIOChannelFileGlib *>(data);
    ...
    if (Channel->DPriorityCallback)
    {
        ...
        if (Channel->DPriorityCallback(Channel->shared_from_this(), EventType,
                                       Channel->DPriorityCalldata))
        {
            return TRUE;
        }
    }
...
gboolean CIOChannelFileGlib::ErrorEventCallback(GIOChannel *source,
                                                GIOCondition condition,
                                                gpointer data)
{
    CIOChannelFileGlib *Channel = static_cast<CIOChannelFileGlib *>(data);
    ...
    if (Channel->DErrorCallback)
    {
        ...
        EventType.SetError();
        if (Channel->DErrorCallback(Channel->shared_from_this(), EventType,
                                    Channel->DErrorCalldata))
        {
            return TRUE;
        }
    }
...
gboolean CIOChannelFileGlib::HangUpEventCallback(GIOChannel *source,
                                                 GIOCondition condition,
                                                 gpointer data)
{
    CIOChannelFileGlib *Channel = static_cast<CIOChannelFileGlib *>(data);
    ...
    if (Channel->DHangUpCallback)
    {
        ...
        EventType.SetHangUp();
        if (Channel->DHangUpCallback(Channel->shared_from_this(), EventType,
                                     Channel->DHangUpCalldata))
        {
            return TRUE;
        }
    }
...
gboolean CIOChannelFileGlib::InvalidEventCallback(GIOChannel *source,
                                                  GIOCondition condition,
                                                  gpointer data)
{
    CIOChannelFileGlib *Channel = static_cast<CIOChannelFileGlib *>(data);
    ...
    if (Channel->DInvalidCallback)
    {
        ...
        if (Channel->DInvalidCallback(Channel->shared_from_this(), EventType,
                                      Channel->DInvalidCalldata))
        {
            return TRUE;
        }
    }
...
void CIOChannelFileGlib::SetInEventCallback(TIOCalldata calldata,
                                            TIOCallback callback)
{
...
void CIOChannelFileGlib::SetOutEventCallback(TIOCalldata calldata,
                                             TIOCallback callback)
{
    ...
    if (callback)
    {
        ...
        {
            DOutTag =
                g_io_add_watch(DChannel, G_IO_OUT, OutEventCallback, this);
        }
    }
    else if (DOutTag)
    {
...
void CIOChannelFileGlib::SetPriorityEventCallback(TIOCalldata calldata,
                                                  TIOCallback callback)
{
    ...
    if (callback)
    {
        ...
        {
            DPriorityTag =
                g_io_add_watch(DChannel, G_IO_PRI, PriorityEventCallback, this);
        }
    }
...
void CIOChannelFileGlib::SetErrorEventCallback(TIOCalldata calldata,
                                               TIOCallback callback)
{
    ...
    if (callback)
    {
        ...
        {
            DErrorTag =
                g_io_add_watch(DChannel, G_IO_ERR, ErrorEventCallback, this);
        }
    }
    else if (DErrorTag)
    {
...
void CIOChannelFileGlib::SetHangUpEventCallback(TIOCalldata calldata,
                                                TIOCallback callback)
{
    ...
    if (callback)
    {
        ...
        {
            DHangUpTag =
                g_io_add_watch(DChannel, G_IO_HUP, HangUpEventCallback, this);
        }
    }
    else if (DHangUpTag)
    {
...
void CIOChannelFileGlib::SetInvalidEventCallback(TIOCalldata calldata,
                                                 TIOCallback callback)
{
    ...
    if (callback)
    {
        ...
        {
            DInvalidTag =
                g_io_add_watch(DChannel, G_IO_NVAL, InvalidEventCallback, this);
        }
    }
    else if (DInvalidTag)
    {
