...
CSoundEventRenderer::CSoundEventRenderer(
    std::shared_ptr<CSoundLibraryMixer> mixer,
    std::shared_ptr<CPlayerData> player)
{
    ...
    DTockIndex = DSoundMixer->FindClip("tock");
    {
        ...
        else
        {
            ...
            while (true)
            {
                ClipIndex = DSoundMixer->FindClip("construct" +
    ...
    for (int TypeIndex = 0; TypeIndex < to_underlying(EAssetType::Max);
         TypeIndex++)
    {
        auto AssetType =
            DPlayer->AssetTypes()
                ->find(CPlayerAssetType::TypeToName((EAssetType) TypeIndex))
                ->second;
        int ClipIndex;
        ...
        {
            DWorkCompleteIndices[TypeIndex].push_back(
                DWorkCompleteIndices[to_underlying(EAssetType::None)].front());
        }
        ...
        else
        {
            ...
            {
                ClipIndex =
                    DSoundMixer->FindClip(Names[TypeIndex] + "-selected" +
                                          std::to_string(SoundIndex));
                if (0 > ClipIndex)
                {
                    break;
            ...
            if (!DSelectionIndices[TypeIndex].size() && AssetType->Speed() &&
                DSelectionIndices[to_underlying(EAssetType::None)].size())
            {
                DSelectionIndices[TypeIndex] =
                    DSelectionIndices[to_underlying(EAssetType::None)];
            }
        }
        ...
        else
        {
            ...
            {
                ClipIndex =
                    DSoundMixer->FindClip(Names[TypeIndex] + "-acknowledge" +
                                          std::to_string(SoundIndex));
                if (0 > ClipIndex)
                {
                    break;
            ...
            if (!DAcknowledgeIndices[TypeIndex].size() &&
                DAcknowledgeIndices[to_underlying(EAssetType::None)].size())
            {
                DAcknowledgeIndices[TypeIndex] =
                    DAcknowledgeIndices[to_underlying(EAssetType::None)];
            }
        }
        ...
        else
        {
            ...
            if (!DDeathIndices[TypeIndex].size() &&
                DDeathIndices[to_underlying(EAssetType::None)].size())
            {
                DDeathIndices[TypeIndex] =
                    DDeathIndices[to_underlying(EAssetType::None)];
            }
        }
        ...
        else
        {
            ...
            if (!DAttackedIndices[TypeIndex].size() &&
                DAttackedIndices[to_underlying(EAssetType::None)].size())
            {
                DAttackedIndices[TypeIndex] =
                    DAttackedIndices[to_underlying(EAssetType::None)];
            }
        }
        if ((to_underlying(EAssetType::Archer) == TypeIndex) ||
            (to_underlying(EAssetType::Ranger) == TypeIndex) ||
            (to_underlying(EAssetType::GuardTower) == TypeIndex))
        {
            DMissleFireIndices[TypeIndex].push_back(
                DSoundMixer->FindClip("bowfire"));
            DMissleHitIndices[TypeIndex].push_back(
                DSoundMixer->FindClip("bowhit"));
        }
        else if (to_underlying(EAssetType::CannonTower) == TypeIndex)
        {
            DMissleFireIndices[TypeIndex].push_back(
                DSoundMixer->FindClip("cannonfire"));
            DMissleHitIndices[TypeIndex].push_back(
                DSoundMixer->FindClip("cannonhit"));
        }
        ...
        else
        {
            ...
            if (!DHarvestIndices[TypeIndex].size() &&
                DHarvestIndices[to_underlying(EAssetType::None)].size())
            {
                DHarvestIndices[TypeIndex] =
                    DHarvestIndices[to_underlying(EAssetType::None)];
            }
        }
        if (1 == AssetType->Range())
        {
            ...
            else
            {
                ...
                {
                    ClipIndex = DSoundMixer->FindClip(
                        "melee-hit" + std::to_string(SoundIndex));
                    if (0 > ClipIndex)
                    {
                        break;
                    }
                ...
                if (!DMeleeHitIndices[TypeIndex].size() &&
                    DMeleeHitIndices[to_underlying(EAssetType::None)].size())
                {
                    DMeleeHitIndices[TypeIndex] =
                        DMeleeHitIndices[to_underlying(EAssetType::None)];
                }
            }
        }
    }
...
float CSoundEventRenderer::RightBias(const SRectangle &viewportrect,
...
bool CSoundEventRenderer::OnScreen(const SRectangle &viewportrect,
                                   const CPosition &position)
{
    ...
    RightX = viewportrect.DXPosition + viewportrect.DWidth +
             CPosition::TileWidth() - 1;
    if (position.X() > RightX)
    {
        return false;
    }
    ...
    BottomY = viewportrect.DYPosition + viewportrect.DHeight +
              CPosition::TileHeight() - 1;
...
void CSoundEventRenderer::RenderEvents(const SRectangle &viewportrect)
{
    ...
    AllEvents.insert(AllEvents.end(), DPlayer->GameEvents().begin(),
                     DPlayer->GameEvents().end());
    for (auto Event : AllEvents)
    {
        if (EEventType::Selection == Event.DType)
        {
            if (Event.DAsset)
            {
                if ((EPlayerNumber::Neutral == Event.DAsset->Number()) ||
                    (DPlayer->Number() == Event.DAsset->Number()))
                {
                    if (EAssetAction::Construct == Event.DAsset->Action())
                    {
                        ...
                        {
                            int RandomClip =
                                MainRandomNumber % DConstructIndices.size();

                            DSoundMixer->PlayClip(
                                DConstructIndices[RandomClip], DVolume,
                                RightBias(viewportrect,
                                          Event.DAsset->Position()));
                        }
                    }
                    else if (DSelectionIndices[to_underlying(
                                                   Event.DAsset->Type())]
                                 .size())
                    {
                        ...
                        {
                            int RandomClip =
                                DRandomNumberGenerator.Random() %
                                DSelectionIndices[to_underlying(
                                                      Event.DAsset->Type())]
                                    .size();
                            if (0 > DDelayedSelectionIndices[to_underlying(
                                        Event.DAsset->Type())])
                            {
                                DDelayedSelectionIndices[to_underlying(
                                    Event.DAsset->Type())] = RandomClip;
                            }
                            else
                            {
                                RandomClip =
        ...
        else if (EEventType::Acknowledge == Event.DType)
        {
            if (Event.DAsset)
            {
                if ((EPlayerNumber::Neutral == Event.DAsset->Number()) ||
                    (DPlayer->Number() == Event.DAsset->Number()))
                {
                    if (DAcknowledgeIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        if (1 >
                            Acknowledges[to_underlying(Event.DAsset->Type())])
                        {
                            int RandomClip =
                                MainRandomNumber %
                                DAcknowledgeIndices[to_underlying(
                                                        Event.DAsset->Type())]
                                    .size();

                            if (0 > DDelayedAcknowledgeIndices[to_underlying(
                                        Event.DAsset->Type())])
                            {
                                DDelayedAcknowledgeIndices[to_underlying(
                                    Event.DAsset->Type())] = RandomClip;
                            }
                            else
                            {
        ...
        else if (EEventType::WorkComplete == Event.DType)
        {
            if (Event.DAsset)
            {
                if (DPlayer->Number() == Event.DAsset->Number())
                {
                    if (DWorkCompleteIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DWorkCompleteIndices[to_underlying(
                                                     Event.DAsset->Type())]
                                .size();
                        ...
                        DSoundMixer->PlayClip(
                            DWorkCompleteIndices[to_underlying(
                                Event.DAsset->Type())][RandomClip],
                            DVolume,
                            RightBias(viewportrect, Event.DAsset->Position()));
                    }
                }
            }
        }
        else if (EEventType::Ready == Event.DType)
        {
            if (Event.DAsset)
            {
                if ((EPlayerNumber::Neutral == Event.DAsset->Number()) ||
                    (DPlayer->Number() == Event.DAsset->Number()))
                {
                    if (DReadyIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DReadyIndices[to_underlying(Event.DAsset->Type())]
                                .size();
        ...
        else if (EEventType::Death == Event.DType)
        {
            if (Event.DAsset)
            {
                ...
                {
                    if (DDeathIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DDeathIndices[to_underlying(Event.DAsset->Type())]
                                .size();
        ...
        else if (EEventType::Attacked == Event.DType)
        {
            if (Event.DAsset)
            {
                if (!OnScreen(viewportrect, Event.DAsset->Position()))
                {
                    if (DAttackedIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DAttackedIndices[to_underlying(Event.DAsset->Type())]
                                .size();

                        DSoundMixer->PlayClip(
                            DAttackedIndices[to_underlying(
                                Event.DAsset->Type())][RandomClip],
                            DVolume,
                            RightBias(viewportrect, Event.DAsset->Position()));
                    }
        ...
        else if (EEventType::MissleFire == Event.DType)
        {
            if (Event.DAsset)
            {
                if (OnScreen(viewportrect, Event.DAsset->Position()))
                {
                    if (DMissleFireIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DMissleFireIndices[to_underlying(
                                                   Event.DAsset->Type())]
                                .size();
        ...
        else if (EEventType::MissleHit == Event.DType)
        {
            if (Event.DAsset)
            {
                if (OnScreen(viewportrect, Event.DAsset->Position()))
                {
                    auto CreationCommand =
                        Event.DAsset->NextCommand();  // Find out type of missle

                    if ((EAssetAction::Construct == CreationCommand.DAction) &&
                        CreationCommand.DAssetTarget)
                    {
                        if (DMissleHitIndices[to_underlying(
                                                  CreationCommand.DAssetTarget
                                                      ->Type())]
                                .size())
                        {
                            int RandomClip =
                                DRandomNumberGenerator.Random() %
                                DMissleHitIndices[to_underlying(
                                                      CreationCommand
                                                          .DAssetTarget->Type())]
                                    .size();
                            ...
                            DSoundMixer->PlayClip(
                                DMissleHitIndices[to_underlying(
                                    CreationCommand.DAssetTarget->Type())]
                                                 [RandomClip],
                                DVolume,
                                RightBias(viewportrect,
                                          Event.DAsset->Position()));
        ...
        else if (EEventType::Harvest == Event.DType)
        {
            ...
            {
                if (OnScreen(viewportrect, Event.DAsset->Position()) &&
                    (Event.DAsset->AttackSteps() - 1 ==
                     (Event.DAsset->Step() % Event.DAsset->AttackSteps())))
                {
                    if (DHarvestIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DHarvestIndices[to_underlying(Event.DAsset->Type())]
                                .size();
        ...
        else if (EEventType::MeleeHit == Event.DType)
        {
            if (Event.DAsset)
            {
                if (OnScreen(viewportrect, Event.DAsset->Position()))
                {
                    if (DMeleeHitIndices[to_underlying(Event.DAsset->Type())]
                            .size())
                    {
                        int RandomClip =
                            DRandomNumberGenerator.Random() %
                            DMeleeHitIndices[to_underlying(Event.DAsset->Type())]
                                .size();

                        DSoundMixer->PlayClip(
                            DMeleeHitIndices[to_underlying(
                                Event.DAsset->Type())][RandomClip],
                            DVolume,
                            RightBias(viewportrect, Event.DAsset->Position()));
                    }
