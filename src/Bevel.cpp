...
CBevel::CBevel(std::shared_ptr<CGraphicTileset> tileset)
{
    ...
    {
        DTopIndices[Index] =
            DTileset->FindTile(std::string("t") + std::to_string(Index));
    }
    ...
    for (int Index = 1; Index < DWidth; Index++)
    {
        DBottomIndices[Index] =
            DTileset->FindTile(std::string("b") + std::to_string(Index));
    }
    ...
    {
        DLeftIndices[Index] =
            DTileset->FindTile(std::string("l") + std::to_string(Index));
    }
    ...
    {
        DRightIndices[Index] =
            DTileset->FindTile(std::string("r") + std::to_string(Index));
    }
...
void CBevel::DrawBevel(std::shared_ptr<CGraphicSurface> surface, int xpos,
                       int ypos, int width, int height)
{
    ...
    for (int XOff = 0; XOff < width; XOff += DWidth)
    {
        ...
        DTileset->DrawTile(surface, xpos + XOff, BottomY,
                           DBottomIndices[Index]);
    }
