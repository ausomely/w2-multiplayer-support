...
{
    int Value;
    if ((0 > to_underlying(dir2)) ||
        (to_underlying(EDirection::Max) <= to_underlying(dir2)))
    {
        return false;
    }
    Value = ((to_underlying(EDirection::Max) + to_underlying(dir2)) -
             to_underlying(dir1)) %
                                 ...
                                 const CPixelPosition &target)
{
    ...
    EDirection SearchDirecitons[] = {EDirection::North, EDirection::East,
    ...
    for (auto &Res : resmap.Assets())
    {
        if (&asset != Res.get())
        {
            ...
            {
                if ((EAssetAction::Walk != Res->Action()) ||
                    (asset.Number() != Res->Number()))
                {
                         ...
                         (EAssetAction::MineGold != Res->Action())))
                    {
                        for (int YOff = 0; YOff < Res->Size(); YOff++)
                        {
                            ...
                            {
                                DMap[Res->TilePositionY() + YOff + 1]
                                    [Res->TilePositionX() + XOff + 1] =
                ...
                {
                    DMap[Res->TilePositionY() + 1][Res->TilePositionX() + 1] =
                        SEARCH_STATUS_OCCUPIED -
                        to_underlying(Res->Direction());
                }
            }
        }
    }
    ...
    CurrentSearch.DTargetDistanceSquared = BestSearch.DTargetDistanceSquared =
        CurrentTile.DistanceSquared(TargetTile);
    ...
    while (true)
    {
        ...
        if (CurrentSearch.DTargetDistanceSquared <
            BestSearch.DTargetDistanceSquared)
        {
            BestSearch = CurrentSearch;
        }
        for (int Index = 0; Index < SearchDirectionCount; Index++)
        {
            ...
            if ((SEARCH_STATUS_UNVISITED ==
                 DMap[TempTile.Y() + 1][TempTile.X() + 1]) ||
                MovingAway(
                    SearchDirecitons[Index],
                    (EDirection)(SEARCH_STATUS_OCCUPIED -
                                 DMap[TempTile.Y() + 1][TempTile.X() + 1])))
            {
                DMap[TempTile.Y() + 1][TempTile.X() + 1] = Index;
                CTerrainMap::ETileType CurTileType =
                    resmap.TileType(TempTile.X(), TempTile.Y());
                // if((CTerrainMap::ETileType::Grass ==
                // CurTileType)||(CTerrainMap::ETileType::Dirt ==
                ...
                if (CTerrainMap::IsTraversable(CurTileType, assetNC,AssetOnTree))
                {
                    ...
                    TempSearch.DTargetDistanceSquared =
                        TempTile.DistanceSquared(TargetTile);
    ...
    {
        CTerrainMap::ETileType CurTileType = resmap.TileType(
            StartX + DiagCheckXOffset[to_underlying(DirectionBeforeLast)],
            StartY + DiagCheckYOffset[to_underlying(DirectionBeforeLast)]);
        // if((CTerrainMap::ETileType::Grass ==
        // CurTileType)||(CTerrainMap::ETileType::Dirt ==
        // CurTileType)||(CTerrainMap::ETileType::Stump ==
        // CurTileType)||(CTerrainMap::ETileType::Rubble ==
        ...
        if (CTerrainMap::IsTraversable(CurTileType, assetNC,AssetOnTree))
        {
            ...
            if ((6 == Sum) && ((EDirection::North == LastInDirection) ||
                               (EDirection::North == DirectionBeforeLast)))
            {  // NW wrap around
                Sum += 8;
            }
            Sum /= 2;
