    ...
    for(auto &Res : resmap.Assets()){
        if(&asset != Res.get()){
            if(EAssetType::None != Res->Type()){
                if((EAssetAction::Walk != Res->Action())||(asset.Number() != Res->Number())){
                    if((asset.Number() != Res->Number())||((EAssetAction::ConveyGold != Res->Action())&&(EAssetAction::ConveyLumber != Res->Action())&&(EAssetAction::MineGold != Res->Action()))){
