...
bool CGraphicMulticolorTileset::LoadTileset(
    std::shared_ptr<CGraphicRecolorMap> colormap,
    std::shared_ptr<CDataSource> source)
{
    ...
    {
        DColoredTilesets.push_back(
            colormap->RecolorSurface(ColIndex, DSurfaceTileset));
    }

    return true;
}

void CGraphicMulticolorTileset::DrawTile(
    std::shared_ptr<CGraphicSurface> surface, int xpos, int ypos, int tileindex,
    int colorindex)
{
    ...
    surface->Draw(DColoredTilesets[colorindex], xpos, ypos, DTileWidth,
                  DTileHeight, 0, tileindex * DTileHeight);
}
