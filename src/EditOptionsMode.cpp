...
void CEditOptionsMode::Render(std::shared_ptr<CApplicationData> context)
{
    ...
    context->DButtonRenderer->Height(context->DButtonRenderer->Height() * 3 /
                                     2);
    ButtonLeft =
        BufferWidth - context->DBorderWidth - context->DButtonRenderer->Width();
    ButtonSkip = context->DButtonRenderer->Height() * 3 / 2;
    ButtonTop = BufferHeight - context->DBorderWidth -
                (DButtonTexts.size() * ButtonSkip -
                 context->DButtonRenderer->Height() / 2);
    ...
    for (auto Text : DButtonTexts)
    {
        ...
        context->DButtonRenderer->DrawButton(
            context->DWorkingBufferSurface, ButtonLeft, ButtonTop, ButtonState);
        if (CButtonRenderer::EButtonState::Inactive != ButtonState)
        {
            DButtonLocations.push_back(SRectangle(
    ...
    OptionTop = (BufferHeight + TitleHeight) / 2 -
    ...
    for (int Index = 0; Index < DEditTitles.size(); Index++)
    {
        ...
        TextOffsetY =
            context->DOptionsEditRenderer->Height() / 2 - TextHeight / 2;
        context
            ->DFonts[to_underlying(CUnitDescriptionRenderer::EFontSize::Large)]
        ...
        context->DOptionsEditRenderer->Text(
            DEditText[Index],
            DEditValidationFunctions[Index](DEditText[Index]));
        context->DOptionsEditRenderer->DrawEdit(
            context->DWorkingBufferSurface, BufferCenter, OptionTop,
            Index == DEditSelected ? DEditSelectedCharacter : -1);
        DEditLocations.push_back(SRectangle(
            {BufferCenter, OptionTop, context->DOptionsEditRenderer->Width(),
