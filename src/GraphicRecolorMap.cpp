...
uint32_t CGraphicRecolorMap::RecolorPixels(void *data, uint32_t pixel)
{
    ...
    if (Alpha)
    {
        ...
        return ((((pixel & 0x00FF0000) * AlphaMult) / 255) & 0x00FF0000) |
               ((((pixel & 0x0000FF00) * AlphaMult) / 255) & 0x0000FF00) |
               ((((pixel & 0x000000FF) * AlphaMult) / 255) & 0x000000FF) |
               Alpha;
    }
    return 0x00000000;
}
...
bool CGraphicRecolorMap::Load(std::shared_ptr<CDataSource> source)
{
    ...
    ColorSurface->Transform(ColorSurface, 0, 0, -1, -1, 0, 0, (void *) this,
                            ObservePixels);
...
std::shared_ptr<CGraphicSurface> CGraphicRecolorMap::RecolorSurface(
    int index, std::shared_ptr<CGraphicSurface> srcsurface)
{
    ...
    DState = index;
    auto RecoloredSurface = CGraphicFactory::CreateSurface(
        srcsurface->Width(), srcsurface->Height(), srcsurface->Format());
    RecoloredSurface->Transform(srcsurface, 0, 0, -1, -1, 0, 0, (void *) this,
                                RecolorPixels);
