...
CMiniMapRenderer::CMiniMapRenderer(std::shared_ptr<CMapRenderer> maprender,
                                   std::shared_ptr<CAssetRenderer> assetrender,
                                   std::shared_ptr<CFogRenderer> fogrender,
                                   std::shared_ptr<CViewportRenderer> viewport,
                                   CGraphicSurface::ESurfaceFormat format)
{
    ...
    DWorkingSurface = CGraphicFactory::CreateSurface(
        DMapRenderer->MapWidth(), DMapRenderer->MapHeight(), format);

    auto ResourceContext = DWorkingSurface->CreateResourceContext();
    ResourceContext->SetSourceRGB(0x000000);
    ResourceContext->Rectangle(0, 0, DMapRenderer->MapWidth(),
                               DMapRenderer->MapHeight());
...
void CMiniMapRenderer::DrawMiniMap(std::shared_ptr<CGraphicSurface> surface)
{
    ...
    if (MMH_MW > MMW_MH)
    {
        ...
        DVisibleHeight = (DMapRenderer->MapHeight() * MiniMapWidth) /
                         DMapRenderer->MapWidth();
    }
    else if (MMH_MW < MMW_MH)
    {
        DVisibleWidth = (DMapRenderer->MapWidth() * MiniMapHeight) /
                        DMapRenderer->MapHeight();
    ...
    /*
    DWorkingPixbuf = gdk_pixbuf_get_from_drawable(DWorkingPixbuf,
    DWorkingPixmap, nullptr, 0, 0, 0, 0, -1, -1);

    ScaledPixbuf = gdk_pixbuf_scale_simple(DWorkingPixbuf, DVisibleWidth,
    DVisibleHeight, GDK_INTERP_BILINEAR); gdk_draw_pixbuf(drawable, TempGC,
    ScaledPixbuf, 0, 0, 0, 0, -1, -1, GDK_RGB_DITHER_NONE, 0, 0);
    ...
    */
    if (nullptr != DViewportRenderer)
    {
        ...
        MiniMapViewportX = (DViewportRenderer->ViewportX() * DVisibleWidth) /
                           DMapRenderer->DetailedMapWidth();
        MiniMapViewportY = (DViewportRenderer->ViewportY() * DVisibleHeight) /
                           DMapRenderer->DetailedMapHeight();
        MiniMapViewportWidth =
            (DViewportRenderer->LastViewportWidth() * DVisibleWidth) /
            DMapRenderer->DetailedMapWidth();
        MiniMapViewportHeight =
            (DViewportRenderer->LastViewportHeight() * DVisibleHeight) /
            DMapRenderer->DetailedMapHeight();
        ResourceContext->Rectangle(MiniMapViewportX, MiniMapViewportY,
                                   MiniMapViewportWidth, MiniMapViewportHeight);
        ResourceContext->Stroke();
        // gdk_draw_rectangle(drawable, TempGC, FALSE, MiniMapViewportX,
        // MiniMapViewportY, MiniMapViewportWidth,  MiniMapViewportHeight);
    }
}
