...
void CAssetRenderer::UpdateLoadingPlayerColors(const std::array< EPlayerColor, to_underlying(EPlayerNumber::Max) > &newcolors){
...
void CAssetRenderer::DrawMiniAssets(std::shared_ptr<CGraphicSurface> surface){
    ...
    if(nullptr != DPlayerData){
        ...
        for(auto &AssetIterator : DPlayerMap->AssetInitializationList()){
            EPlayerColor AssetColor = DLoadingPlayerColors[to_underlying(AssetIterator.DNumber)];
